...
exports.BUDGET_RANGES = exports.LOCATIONS = exports.STREAMS = exports.COLLEGES = void 0;
exports.getMatchLabel = getMatchLabel;
exports.filterColleges = filterColleges;
exports.sortColleges = sortColleges;
...
function getMatchLabel(studentPercentile, collegeMinPercentile) {
    var gap = studentPercentile - collegeMinPercentile;
    if (gap >= 10)
        return "Safe";
    if (gap >= 0)
        return "Good Match";
...
function filterColleges(profile) {
    var _a, _b;
    var budgetMax = (_b = (_a = exports.BUDGET_RANGES.find(function (b) { return b.value === profile.budget; })) === null || _a === void 0 ? void 0 : _a.max) !== null && _b !== void 0 ? _b : Infinity;
    return exports.COLLEGES.filter(function (c) {
        var percentileOk = profile.percentile >= c.minPercentile;
        var streamOk = c.streams.includes(profile.stream);
        var locationOk = profile.location === "Any" || c.location === profile.location || c.location === "Any";
        var budgetOk = c.fees <= budgetMax;
        return percentileOk && streamOk && locationOk && budgetOk;
    });
}
function sortColleges(colleges, sort) {
    return __spreadArray([], colleges, true).sort(function (a, b) {
        if (sort === "rating")
...
function applyClientFilters(colleges, filters) {
    var _a, _b, _c;
    var name = ((_a = filters.name) !== null && _a !== void 0 ? _a : "").trim().toLowerCase();
    var minRating = (_b = filters.minRating) !== null && _b !== void 0 ? _b : 0;
    var type = (_c = filters.type) !== null && _c !== void 0 ? _c : "any";
    return colleges.filter(function (c) {
        if (name && !c.name.toLowerCase().includes(name))
            return false;
        if (c.rating < minRating)
            return false;
        if (type !== "any" && c.type !== type)
            return false;
        return true;
    });
}
